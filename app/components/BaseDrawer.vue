<template>
  <Teleport to="body">
    ...
    <div v-if="shown" class="fixed inset-0 z-[80]">
      <!-- Overlay -->
      <div class="absolute inset-0 bg-black/50 backdrop-blur-[2px] transition-opacity duration-300"
        :style="{ opacity: overlayOpacity }" :class="overlayClass" @click="onOverlayClick" />
      ...
      <section ref="panel"
        ...
        <!-- Header -->
        <header v-if="showHeader" class="flex items-center justify-between gap-3 px-4 py-3">
          <div class="min-w-0">
          ...
          <button v-if="closable" type="button" class="rounded-lg p-2 hover:bg-white/10 active:scale-[0.98] transition"
            aria-label="Close drawer" data-drawer-no-drag @click="requestClose()">
...
const panelPosClass = computed(() => {
  switch (props.side) {
    ...
    case "top": return "left-0 right-0 top-0"
...
function onPointerMove(e: PointerEvent) {
  ...
  const dAxis =
    axis.value === "x" ? e.clientX - drag.value.lastX : e.clientY - drag.value.lastY
