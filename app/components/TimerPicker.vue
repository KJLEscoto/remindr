<template>
  <div class="relative" ref="root">
    ...
    <input
      ...
      :value="display" placeholder="--:--:--" @click="toggle" readonly
      @focus="($event.target as HTMLInputElement).blur()" aria-label="Selected time" />
      ...
      <Teleport to="body">
        <div v-if="open" ref="panel"
          ...
          <div class="grid grid-cols-3 gap-2">
            <section class="group">
              ...
              <div class="relative">
                <select v-model="draftHour"
                  ...
                  <option class="text-black" v-for="h in hours" :key="h" :value="h">
                    {{ h }}
                  </option>
                </select>
                <span
                  ...
                  <ChevronsUpDown class="size-4 text-white pointer-events-none" />
                </span>
              </div>
            </section>
            ...
            <section class="group">
              ...
              <div class="relative">
                <select v-model="draftMinute"
                  class="appearance-none w-full rounded-full bg-white/10 border border-white/10 px-3 py-2 outline-none cursor-pointer">
                  <option class="text-black" value="--">--</option>
                  <option class="text-black" v-for="m in minutes" :key="m" :value="m">
                    {{ m }}
                  </option>
                </select>
                <span
            ...
            <section class="group">
              <p class="mb-1 text-xs text-white/60 px-1">Seconds</p>
              <div class="relative">
                <select v-model="draftSecond"
                  class="appearance-none w-full rounded-full bg-white/10 border border-white/10 px-3 py-2 outline-none cursor-pointer">
                  <option class="text-black" value="--">--</option>
                  <option class="text-black" v-for="s in seconds" :key="s" :value="s">
                    {{ s }}
                  </option>
                </select>
                <span
          ...
          <div class="mt-3 flex items-center justify-end gap-2 border-t border-white/5 pt-3">
            ...
            <button type="button"
              ...
              @click="applyAndClose" :disabled="!canApply">
              Apply
            </button>
          </div>
        </div>
...
const savedSecond = ref("--");

const draftHour = ref("--");
const draftMinute = ref("--");
const draftSecond = ref("--");
...
function parseInitial(v: string) {
  // accepts "HH:MM:SS" (also tolerates "H:M:S")
  const match = v.match(/^(\d{1,2}):(\d{1,2}):(\d{1,2})$/);
  if (!match) return;

  const hh = String(match[1] ?? "00").padStart(2, "0");
  const mm = String(match[2] ?? "00").padStart(2, "0");
  const ss = String(match[3] ?? "00").padStart(2, "0");

  savedHour.value = hh;
  savedMinute.value = mm;
  savedSecond.value = ss;
}
...
const display = computed(() => {
  // show saved values (always 00-..)
  return `${savedHour.value}:${savedMinute.value}:${savedSecond.value}`;
});
...
async function toggle() {
  if (!open.value) {
    // start drafts as "unset" each open
    draftHour.value = "--";
    draftMinute.value = "--";
    draftSecond.value = "--";
...
const canApply = computed(() => {
  // enable if user selected at least one field
  return draftHour.value !== "--" || draftMinute.value !== "--" || draftSecond.value !== "--";
});

function applyAndClose() {
  // If a field isn't selected, treat it as 00
  savedHour.value = norm(draftHour.value);
  savedMinute.value = norm(draftMinute.value);
  savedSecond.value = norm(draftSecond.value);
...
function onDocClick(e: MouseEvent) {
  ...
  if ((root.value && root.value.contains(t)) || (panel.value && panel.value.contains(t))) return;
  open.value = false;
}
...
// If panel content size changes, re-position
watch(open, (v) => {
